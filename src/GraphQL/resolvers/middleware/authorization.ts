...
export const isAuthenticated = (_: any, __: any, { me }: any) =>
	me ? skip : new AuthenticationError('Not authenticated as user. Please login or signup.')
...
export const isAdminOrPartner = combineResolvers(
	isAuthenticated,
	async (_: any, __: any, { me: { id, role }, models }) => {
		/* 
			This step (role === 'ADMIN' || role === 'PARTNER' at very first) 
			may not required but this will make the calculation fast.
			Because if this fails then an additional DB query will not execute
		*/
		if (role === 'ADMIN' || role === 'PARTNER') {
			...
			if (user && (user.role === 'ADMIN' || user.role === 'PARTNER')) {
				...
				*/
				return new AuthenticationError(
					'Your are no longer authorized to perform any admin/partner actions. Your admin/partner account has been deleted.'
				)
			}
		} else {
			return new ForbiddenError('Not authorized neither as admin nor partner.')
		}
	}
)
...
export const isProductOwner = async (_: any, { id }: any, { models, me }: any) => {
	if (!id) throw new UserInputError('Invalid product id.')
	const product = await models.Product.findByPk(id, { raw: true })
	if (!product) throw new UserInputError('No associated product with the given id.')
	if (product.userId !== me.id) {
		throw new ForbiddenError('Not authenticated as owner of this product.')
	}
